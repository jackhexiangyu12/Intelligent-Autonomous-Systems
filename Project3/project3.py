...
def load_data(num, data_path="../Data/ECE5242Proj3-train/"):
    ...
    closest_lidar = [lidar[k] for k in np.argmin(np.abs(np.array(ts).reshape(1, -1) - np.array([l['t'] for l in lidar]).reshape(-1, 1)), axis=0)]
...
def track_global_movement(r, l, lidar_data, w, averaging=1):
    ...
    for index, (rr, ll, scans) in enumerate(zip(r, l, lidar_data[::averaging])):
        ...
        delta = np.array([(rr + ll) * np.cos(current_theta / 2) / 2, (rr + ll) * np.sin(current_theta / 2) / 2]).reshape(-1, 1) * .254 * np.pi / 360
...
def track_global_indices_SLAM(w, x_offset, y_offset, x_range, y_range, indices_per_int, file_num, data_path, num_particles, max_noise, update_threshold, subtract, min_max_cap=1000, averaging=1, testing=False):
    # average right side and left side encoding data and lidar data to match length
    ...
    for index, (rr, ll, scans) in enumerate(zip(r, l, lidar_data[::averaging])):
        phi += np.random.uniform(-max_noise / np.sqrt(index + 1), max_noise / np.sqrt(index + 1), num_particles).reshape(1, num_particles)
        # calculate and update values based on the slides (theta, delta x and y, R, phi)
        current_theta = (rr - ll) / w
        delta = np.array([(rr + ll) * np.cos(current_theta / 2) / 2, (rr + ll) * np.sin(current_theta / 2) / 2]).reshape(-1, 1) * .254 * np.pi / 360
        ...
        # map scanned (wall) values to grid
        scan_x_list = np.rint(((loc_x + np.cos(phi + angles) * scans['scan'].reshape(-1, 1)) + x_offset) * indices_per_int)
        scan_y_list = np.rint(((loc_y + np.sin(phi + angles) * scans['scan'].reshape(-1, 1)) + y_offset) * indices_per_int)
        # map current location to grid
        loc_x_index = np.rint((loc_x + x_offset) * indices_per_int)
        loc_y_index = np.rint((loc_y + y_offset) * indices_per_int)
        ...
        best_gaps = getMapCellsFromRay(loc_x_index[best_robot], loc_y_index[best_robot], scan_x_list[:, best_robot], scan_y_list[:, best_robot], 0).T.astype(int).reshape(-1, 2)
            # temp_indices = np.zeros_like(indices)
            # temp_indices[walls[:, 0], walls[:, 1]] += 0.9
            # temp_indices[gaps[:, 0], gaps[:, 1]] -= 0.7
    ...
    if testing:
        ...
        display_indices(indices, filename="results/SLAM_img" + str(file_num) + "_" + str(int(10 * w)) + "_" + str(min_max_cap) + "_" + str(int(10 * subtract)) + "_" + str(np.abs(update_threshold)) + "_" + str(int(1000 * max_noise)))
    #plt.show()
    # print(indices)
    # #plt.imshow(indices, cmap='hot')
    # ax = sns.heatmap(np.log(np.maximum(0.5, freqs * indices)))
...
def track_global_indices_SLAMSLAM(w, x_offset, y_offset, x_range, y_range, indices_per_int, file_num, data_path, num_particles, max_noise, update_threshold, subtract, min_max_cap=1000, averaging=1, testing=False):
    # average right side and left side encoding data and lidar data to match length
    r1, l1, r2, l2, ts, lidar_data = load_data(file_num, data_path=data_path)
    r = (r1 + r2) / 2
    l = (l1 + l2) / 2
...
def show_paths(w, x_offset, y_offset, x_range, y_range, indices_per_int, file_num, num_particles, max_noise, min_max_cap=1000, averaging=1):
    # average right side and left side encoding data and lidar data to match length
    r1, l1, r2, l2, ts, lidar_data = load_data(file_num)
    r = (r1 + r2) / 2
    l = (l1 + l2) / 2
        ...
        for angle in [0.001]:
            for min_max in [25]: # check smaller?
                for sub in [0.4]:# [0.3,0.4,0.5]: # check smaller?
                    track_global_indices_SLAM(width, 45, 45, 90, 90, 4, filenum, data_path, 50, angle, thresh, sub, min_max_cap=min_max, averaging=4, testing=True)
# import os
